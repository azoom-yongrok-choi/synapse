...
class TonePolishAgent(BaseAgent):
    _llm_agent: LlmAgent = PrivateAttr()

    def __init__(self):
        super().__init__(name="tone_polish_agent")
        self._llm_agent = LlmAgent(
            model=LiteLlm(model="openai/gpt-4o-mini"),
            name="tone_polish_llm",
            instruction="""
Guidelines:
- For every response, always start with a title line: `[Tone Polished by LLM]` (include this exactly, at the very top of your reply).
...
""",
    ...
    async def _run_async_impl(self, ctx):
        logging.info("[TonePolishAgent] Called _run_async_impl")
        async for event in self._llm_agent.run_async(ctx):
            logging.info(f"[TonePolishAgent] Yielding event: {event}")
            yield event
